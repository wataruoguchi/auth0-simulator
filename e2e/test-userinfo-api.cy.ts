describe("UserInfo API Endpoint", () => {
  it("should return user info when called with valid access token", () => {
    ...
    cy.window().then((win) => {
      const auth0Data = win.localStorage.getItem('@@auth0spajs@@::test-client-id::@@user@@');
      ...
      cy.request({
        ...
        expect(response.body).to.have.property('iss', 'https://localhost:4400/');
        expect(response.body).to.have.property('azp', 'test-client-id');
        expect(response.body).to.have.property('scope', 'openid profile email offline_access');
      });
    });
  });
