...
https.get(
  "https://localhost:4400/.well-known/jwks.json",
  ...
  (res) => {
    ...
    res.on("end", () => {
      ...
      https.get(
        "https://localhost:4400/.well-known/openid_configuration",
        { rejectUnauthorized: false },
        (res) => {
          let data = "";
          res.on("data", (chunk) => (data += chunk));
          res.on("end", () => {
            ...
            http.get("http://localhost:3000/", (res) => {
              ...
              res.on("end", () => {
                console.log(
                  "App Response (first 200 chars):",
                  data.substring(0, 200),
                );
                console.log(
                  "App contains login button:",
                  data.includes('data-testid="login-button"'),
                );
              });
            });
          });
